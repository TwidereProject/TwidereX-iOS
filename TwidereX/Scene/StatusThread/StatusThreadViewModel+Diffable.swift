...
extension StatusThreadViewModel {
    ...
    @MainActor func setupDiffableDataSource(
        ...
        // tableView.register(LoaderTableViewCell.self, forCellReuseIdentifier: String(describing: LoaderTableViewCell.self))
        ...
        switch kind {
        case .status(let status):
            // if status.inReplyToUserID != nil {
            //     snapshot.appendItems([.topLoader])
            // }
            break
