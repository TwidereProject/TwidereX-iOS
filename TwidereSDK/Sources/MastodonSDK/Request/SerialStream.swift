...
public final class SerialStream: NSObject {
    ...
    private let workingQueue = DispatchQueue(label: "com.twidere.MastodonSDK.SerialStream.\(UUID().uuidString)")
    ...
    public init(streams: [InputStream]) {
        ...
        writingTimerSubscriber = Timer.publish(every: 0.5, on: .current, in: .default)
            ...
            .sink { [weak self] timer in
                ...
                while readBytesCount < SerialStream.bufferSize {
                    ...
                    let remainsCount = SerialStream.bufferSize - readBytesCount
                    let readCount = inputStream.read(&self.buffer[readBytesCount], maxLength: remainsCount)
